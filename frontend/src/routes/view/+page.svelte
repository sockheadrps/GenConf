<script lang="ts">
  ...
  async function fetchCompletedGenerators(month: string) {
    const uri = `http://127.0.0.1:8000/records/${month.toLowerCase()}`;
  ...
  async function fetchGeneratorData() {
    ...
    const uri = `http://127.0.0.1:8000/gen_data/${selectedMonth.toLowerCase()}/${selectedGenerator}`;
    try {
      const response = await fetch(uri);
      if (!response.ok) {
...
<div class="p-6 bg-gray-900 min-h-screen text-gray-300">
  <h1 class="text-2xl font-bold mb-4">Generator Records</h1>
  ...
  <div class="mb-6">
    ...
    <select
      ...
      class="block mt-2 w-full bg-gray-800 text-gray-300 p-2 rounded focus:outline-none focus:ring-2 focus:ring-gray-700"
    >
  ...
  {#if selectedGenerator && preRunData && postRunData}
    <div class="mt-8">
      <h2 class="text-xl font-bold mb-4">Generator Records</h2>
      
      <!-- Pre-Run Data -->
      <div class="bg-gray-800 rounded-lg p-4 mb-4">
        <h3 class="text-lg font-semibold mb-2">Pre-Run Check</h3>
        <div class="data-row py-2">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p><span class="font-medium">Oil Check:</span> {preRunData.oil_check ? 'Passed' : 'Failed'}</p>
      ...
      <div class="bg-gray-800 rounded-lg p-4">
        <h3 class="text-lg font-semibold mb-2">Post-Run Check</h3>
        <div class="data-row py-2">
          <div class="grid grid-cols-2 gap-4">
            <div>
